...
module.exports = (req, res) => {
  cors(req, res, async () => {
    ...
    try {
      ...
      if (!userDoc.exists) {
        return res.status(200).send({ username: null }); // Kullanıcı bulunamadı
      } else {
        const userData = userDoc.data();
        return res.status(200).send({ username: userData.username });
      }
    } catch (error) {
      console.error("Kullanıcı verileri alınırken hata oluştu:", error);
      return res.status(500).send({
        status: "error",
        message: error.message || "Kullanıcı verileri alınırken bilinmeyen bir hata oluştu."
      });
    }
  });
}; 